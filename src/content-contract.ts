...
export function assertContentSnapshotV1(value: unknown): asserts value is ContentSnapshotV1 {
  ...
  for (let i = 0; i < shops.length; i++) {
    ...
    if (!isString(sc.line)) {
      throw new Error(`${prefix}.data.shops[${i}].contact.line must be a string`);
    }
    if (!isString(sc.lineQr)) {
      throw new Error(`${prefix}.data.shops[${i}].contact.lineQr must be a string`);
    }
    if (!isString(sc.area)) {
      throw new Error(`${prefix}.data.shops[${i}].contact.area must be a string`);
    }
    if (!isString(sc.hours)) {
      throw new Error(`${prefix}.data.shops[${i}].contact.hours must be a string`);
    }
    if (!isString(sc.secondaryLine)) {
      throw new Error(`${prefix}.data.shops[${i}].contact.secondaryLine must be a string`);
    }
    if (!isString(sc.secondaryLineQr)) {
      throw new Error(`${prefix}.data.shops[${i}].contact.secondaryLineQr must be a string`);
    }
    // Note: imageMapKeys not yet defined at this point — QR ref checks
  ...
  const pricePlans = data.pricePlans as Record<string, unknown>[];
  for (let i = 0; i < pricePlans.length; i++) {
    const plan = pricePlans[i];
    if (!isRecord(plan)) {
      throw new Error(`${prefix}.data.pricePlans[${i}] must be an object`);
    }
    if (!isString(plan.name)) {
      throw new Error(`${prefix}.data.pricePlans[${i}].name must be a string`);
    }
    if (!isString(plan.note)) {
      throw new Error(`${prefix}.data.pricePlans[${i}].note must be a string`);
    }
    if (!isArray(plan.rows)) {
      throw new Error(`${prefix}.data.pricePlans[${i}].rows must be an array`);
    }
    for (let r = 0; r < (plan.rows as unknown[]).length; r++) {
      if (!isString((plan.rows as unknown[])[r])) {
        throw new Error(`${prefix}.data.pricePlans[${i}].rows[${r}] must be a string`);
      }
    }
  }
  ...
  const imageMapKeys = new Set<string>();
  for (const [k, v] of Object.entries(imageMap)) {
    if (!isString(v)) {
      throw new Error(`${prefix}.imageMap["${k}"] must be a string`);
    }
    imageMapKeys.add(k);
  }
  ...
  for (let i = 0; i < profiles.length; i++) {
    ...
    // shopId must reference a declared shop
    if (!shopIds.has(p.shopId as string)) {
      throw new Error(`${pPath}.shopId references unknown shop: ${p.shopId}`);
    }
    ...
    if (vids !== undefined && vids !== null) {
      ...
      for (let v = 0; v < vids.length; v++) {
        if (!isString(vids[v])) {
          throw new Error(`${pPath}.videos[${v}] must be a string`);
        }
      }
    }
  }
  ...
  // Validate contact QR image references
  if (isString(contact.lineQr) && contact.lineQr.length > 0) {
    if (!imageMapKeys.has(contact.lineQr)) {
      throw new Error(`${prefix}.data.contact.lineQr references missing image id: ${contact.lineQr}`);
    }
  }
  if (isString(contact.secondaryLineQr) && contact.secondaryLineQr.length > 0) {
    if (!imageMapKeys.has(contact.secondaryLineQr)) {
      throw new Error(`${prefix}.data.contact.secondaryLineQr references missing image id: ${contact.secondaryLineQr}`);
    }
  }
  ...
  for (let i = 0; i < hotels.length; i++) {
    ...
    if (!isRecord(hotel)) {
      throw new Error(`${prefix}.data.hotels[${i}] must be an object`);
    }
  ...
  // Validate shop contact QR image references
  for (let i = 0; i < shops.length; i++) {
    const shop = shops[i] as Record<string, unknown>;
    if (isRecord(shop.contact)) {
      const sc = shop.contact as Record<string, unknown>;
      if (isString(sc.lineQr) && (sc.lineQr as string).length > 0 && !imageMapKeys.has(sc.lineQr as string)) {
        throw new Error(`${prefix}.data.shops[${i}].contact.lineQr references missing image id: ${sc.lineQr}`);
      }
      if (isString(sc.secondaryLineQr) && (sc.secondaryLineQr as string).length > 0 && !imageMapKeys.has(sc.secondaryLineQr as string)) {
        throw new Error(`${prefix}.data.shops[${i}].contact.secondaryLineQr references missing image id: ${sc.secondaryLineQr}`);
      }
    }
  }
