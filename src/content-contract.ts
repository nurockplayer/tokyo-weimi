...
export function assertContentSnapshotV1(value: unknown): asserts value is ContentSnapshotV1 {
  ...
  if (isString(contact.lineQr) && contact.lineQr.length > 0 && !imageMapKeys.has(contact.lineQr)) {
    throw new Error(`${prefix}.data.contact.lineQr references missing image id: ${contact.lineQr}`);
  }
  if (isString(contact.secondaryLineQr) && contact.secondaryLineQr.length > 0 && !imageMapKeys.has(contact.secondaryLineQr)) {
