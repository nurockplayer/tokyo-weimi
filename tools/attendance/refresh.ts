...
const emptyProfileTranslations = (): Record<LanguageCode, Record<string, TranslatedProfileText>> => ({
  "zh-Hant": {},
  "zh-Hans": {},
  ja: {},
  ko: {},
...
const readProfileTranslations = (): Record<LanguageCode, Record<string, TranslatedProfileText>> => {
  const file = path.join(rootDir, "src/content/profile-translations.json");
  if (!existsSync(file)) return emptyProfileTranslations();
  const parsed = JSON.parse(readFileSync(file, "utf8")) as Partial<Record<LanguageCode, Record<string, TranslatedProfileText>>>;
  return {
...
const profileId = (profile: SourceProfile, existingByName: Map<string, Set<string>>): string => {
...
const deriveTitle = (profile: SourceProfile, tags: string[], name: string): string => {
...
const mergeTranslationResponse = (
  ...
  translations: Record<LanguageCode, Record<string, TranslatedProfileText>>,
): void => {
...
const translateProfiles = async (
  profiles: Profile[],
  translations: Record<LanguageCode, Record<string, TranslatedProfileText>>,
): Promise<Record<LanguageCode, Record<string, TranslatedProfileText>>> => {
  ...
  for (const { sourceProfile, media } of enrichedSourceProfiles) {
    ...
    const id =
      sourceProfile.shopId === "ikebukuro-vip"
        ? vipProfileId(sourceProfile, imageIds[0], currentVipNameCounts.get(name) || 1, existingByName, existingById)
        : profileId(sourceProfile, existingByName);
    ...
    profiles.push({
      ...
      name,
      title: previousProfile?.title || deriveTitle(sourceProfile, tags, name),
      ...
      summary: previousProfile?.summary
        ...
        : withFallback(deriveSummary(sourceProfile), deriveTitle(sourceProfile, tags, name)),
      image: gallery[0] || "",
      gallery,
