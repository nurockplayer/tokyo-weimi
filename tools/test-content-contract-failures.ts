 ...
 * Each test constructs a minimal payload that should be rejected,
 * then verifies the rejection message matches the expected reason.
 *
...
test("translation references missing profile id", () => {
  expectThrow(() => {
    const snap = validSnapshot({
      profileTranslations: {
        en: {
          "non-existent-profile": { title: "T", tags: ["t"], summary: "S" },
        },
      },
    });
    assertContentSnapshotV1(snap);
